...
conducted reqr gathering, data mappings, designing and mainting enterprices application 

developed reporting solutions based on the needs of .. identifying x inovative ways to 

created an e2e sol for etl process monitoring using 

automated etl process across a high volume of data which reduced the manual workload

collborated with tech profesionals including product & data science teams ...on idea generation, implementation, testing & success msrmnt

developed a key data ppln to process 5tb of dt by consolidating from multiple disparate srcs into single dest, enabling quick data analysis for reliable biz insights

implemented best practise process & created cloud functions, applications & db that have improved data accuracy for decision making by 35%

built tableau dashboard to vizualise core biz kpis 

aggregated unstructured data from 20+ sources to build the foundation of

designed the data ppline archit 

led the transistion to a paperless practise by implmenting an electroic booking system & a faster & more accurate biz sys, reduced cost of labor & office overhead by 10%
