...
validation only after 12hrs ie if reserv is applied to model, after applied charge will go to reserv

 
 calculate - creates reservation order
 scope: subsciption
 product: should be provision managed Data Zone
 region: should match resource region (openai instance)
 
 ptu number & reservation should match
 
 1st deploy then purchase
 
 spn / oly purchaser can see the purchased prod
------------
